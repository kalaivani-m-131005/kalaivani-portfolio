...
import { Layers, Cpu, CheckCircle2, AlertCircle, Sparkles, Terminal, Code2 } from 'lucide-react';

export const Projects: React.FC = () => {
  const { project } = resumeData;
  const [activeTab, setActiveTab] = useState<'overview' | 'features' | 'architecture' | 'contribution'>('overview');
        ...
        <div className="rounded-3xl glass-panel border border-white/10 overflow-hidden shadow-2xl">
          ...
          <div className="flex border-b border-white/10 bg-black/40 overflow-x-auto">
            <button
              ...
              className={`px-6 py-3.5 text-xs font-mono font-semibold transition-colors flex items-center gap-2 border-b-2 ${
                activeTab === 'overview'
          ...
          <div className="p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="p-5 rounded-2xl bg-white/5 border border-white/10 space-y-3 font-mono text-xs text-gray-300">
                  <div className="flex items-center justify-between pb-2 border-b border-white/10">
                    <span className="text-[#D4AF37] font-bold">SYSTEM ARCHITECTURE COMPONENT MATRIX</span>
                    <span className="text-gray-500">MERN + Gemini Stack</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                    <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                      <span className="block font-bold text-white mb-1">Frontend Layer</span>
                      <span className="text-gray-400 text-[11px]">React.js with TypeScript for strictly typed component UI state.</span>
                    </div>
                    <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                      <span className="block font-bold text-white mb-1">Backend & API Layer</span>
                      <span className="text-gray-400 text-[11px]">Node.js / Express.js scalable RESTful endpoints.</span>
                    </div>
                    <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                      <span className="block font-bold text-white mb-1">AI Intelligence</span>
                      <span className="text-gray-400 text-[11px]">Google Gemini API integration for resume feedback & mock interviews.</span>
                    </div>
                    <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                      <span className="block font-bold text-white mb-1">Database & Auth</span>
                      <span className="text-gray-400 text-[11px]">MongoDB Atlas schemas with JWT role-based access.</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {project.contribution.map((item, i) => (
                    <div key={i} className="p-3.5 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3 text-sm text-gray-200">
            ...
            <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs text-gray-400 font-mono">
              <span>Source repository details available upon verified request.</span>
              <span className="text-[#D4AF37]">Factual Project Spec</span>
            </div>
          </div>
        </div>
      </div>
